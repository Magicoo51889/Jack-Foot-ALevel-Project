...
My objective for this cycle is to modularise the program up, so that the functions that I have are more easily accessible, and easier to debug. This will also help further down the line when I try and add other scenes and backgrounds to the game, as it will make it more obvious to where each module is and it means they're not all bundled together into the scene one file.&#x20;

* [x] Modularise the program
* [x] Prepare for more scenes
...
| Variable Name | Use                                                          |
| ------------- | ------------------------------------------------------------ |
| N/A           | I'm not adding any code for this cycle, simply rearranging.  |
...
The most challenging part of this was getting the data to parse from each file across, as it would occasionally miss off a part of the import that would mean one function wasn't being found or called. It also meant that I had to add the functions directly into the create function in each scene, and not after, which took some time to figure out how to do without messing up the order of the code.&#x20;
...
| ---- | ------------ | ---------------------------------------------- | --------------------- | --------- |
| 1    | Run code     | The game runs, with no errors or missing data. | As expected           | Pass      |

### Evidence

This rearrange has made it much easier to find each function, and makes it easier for me to be able to add more functions in the future, without clogging up one file with lots of the other functions, that I may not need for that scene.&#x20;

Doing this also means I won't accidentally repeat code, as I will be able to import these functions across the whole application making it easier to use everywhere I may need a function.&#x20;
